...
\selectfont{\textbf{\textcolor{orb!75!black}{Bd\resizebox{6.5pt}{!}{Q}S}}}}}}
...
within \PDF{} character streams.   Special-purpose \GUI{} windows can be shown that allow users to 
fine-tune the search keywords and options (see Figure~\ref{fig:ss1} for an example of how such dialog 
windows might appear).}

\p{\lPacTk{} similarly provides tools for compiling indexes.  In addition to listing 
associations between index terms (also called \q{headings}) and page numbers or 
paragraph codes (also called \q{locators}), index metadata should model heading/subheading 
...
authors and editors to examine index metadata systematically, by navigating through headings 
in multiple ways (by order or creation, alphabetical sequence, or following parent/subheading 
and redirect connections) and juxtaposing index-entry windows with \PDF{} page displays 
...
\p{While curating an index, authors will often start with a list of terms which have clear 
relevance for a publication's topic, and then search for instances of those headings in the 
document text.  Although locators may often correspond to paragraphs without an exact match, 
a raw-text search can be a useful means to establish as provisional collection of locators 
that can be extended afterward.}
...
allows users to quickly rearrange or exclude words from the keyphrase to construct the 
...
to be updated with paragraph codes for the new book.  We therefore employed a version of a
...
then new headings (plus new locators for old terms) can be added.}    
...
be used to review two different editions of a book whose index is being updated.  The software then 
has two \PDF{} view windows to show the two editions, plus a central window with a variety of entry lines, buttons, and check boxes, intended to automate the data transfer as much as possible.  After identifying all of the index terms from the prior volume and the corresponding page references, the software is designed to work through index entries one at a time --- either in alphabetized sequence or where users select a specific entry to start with.  For each entry, users can click through the different page matches from the first edition, and the app searches for similar terms in the second edition.  Above-mentioned search features allow users to fine-tune the search key to be more precise if needed --- for instance, inverting a proper name  or restricting the terms to the most important words (e.g., \q{`Diagnostic Impression' chart} might be reduced to \q{Diagnostic Impression}).  Making these adjustments by clicking buttons is quicker and easier for the user than typing new search terms manually.}

\p{Once search terms are matched against the new document, the code shows matches that are highlighted in both the first and second edition, and also shows the plain text of the corresponding files in separate windows.  It is worth mentioning that the highlights are constructed via native \GUI{} overlays, rather than employing the internal \PDF{} highlight mechanism --- the former is more flexible, and allows features like semi-transparency and color variation (which prove quite useful during a session where multiple entries and/or term-variants have been processed).  With these features, the user is able to quickly check whether a match replicates an entry in the earlier index or else is a new locator.  In either case the user will click to register the match for the new document's index and track the correlation between the old and new index (the software automatically identifies correct paragraph codes if those codes, rather than page numbers, are used to generate an index).  When ready, the user can confirm that the data for the current heading is complete and proceed to the next one.  The software shown in Figure~\ref{fig:ss3} then generates \HTML{} code for that entry, which ultimately gets folded into the compiled second-edition index (this might be in formats such as Word, \PDF{}, \XML{}, etc.).  Users have the option of automatically storing the entry data on a web back-end, or via \FTP{}, which comes in handy if multiple people are working on an index at the same time.}

\p{Visible in Figure~\ref{fig:ss2} and Figure~\ref{fig:ss3} are many \GUI{} controls through which users can work through a list of entries via buttons, check boxes, context menus, and \q{dock} widgets (components that will either be affixed to the main window or float as separate windows, as desired).  Some of these controls help to quickly process individual entries; others affect the search process by filtering the target search area (e.g., restricting to certain pages --- such as a specific chapter --- or just to footnotes, etc.).  These various \GUI{} features enable the user to process a single entry much more quickly that manual text-editing.  Given a project for a second-edition, for instance, often it is possible to confirm a match between the earlier and later document, and update the later index accordingly, in just a few seconds.}

\p{Whether building an index from scratch or updating an old manuscript, these \GUI{} capabilities streamline the indexing process.  Rather than typing index entries directly into an index file --- manually adjusting details like italics, subentry lists, roman vs. arabic numerals, and so forth --- the user could rely on mouse clicks and context menus (with minimal actual typing) to compile a data structure holding all index entries.  A human-readable index is then generated from that structured data, or authors could submit the index as a data file instead, which could be incorporated into Meta-Indexes discussed below.}
...
for example, syncing an index to a published data set; or compiling an aggregate index from a volume with individual chapter authors.  Modular, native-compiled \GUI{} controls allow new secondary windows to be implemented so as 
...
content from a different chapter or section of the same book/article.  This would allow side-by-side 
...
\vspace*{5pt}
%\sectnum{1.}
\hspace{2pt}\secttitle{{Meta-Indexing and Cloud Services}}
\vspace*{5pt}
{\semihr{0.27\textwidth}}
...
\p{\PDF{} annotation capabilities (which include editing and comment boxes) are unfortunately very limited.  
Many \PDF{} viewers cannot show annotations at all; those that do often present annotations in small, 
hard-to-read boxes.  One solution to this problem is to convert \PDF{} documents to a series of 
\SVG{} files, that can be browsed as individual pages.  \SVG{} is a general-purpose format for 
...
Figure~\ref{fig:ss4} shows a feature where annotated \PDF{}s are converted to \SVG{} files so as to better visualize the annotations (which in ordinary \PDF{} viewers tend to be reduced to poorly-formatted comment boxes).  Here the comment-text 
...
a master index was then compiled linking specific pages to the annotation categories, with hyperref 
...
on the other hand, each page can have its own web \URL{}, and links to single pages can be 
embedded in web pages (and also other \PDF{} documents).  Such web links can be useful for collaborative 
...
\p{\lPacTk{} also supports technology for embedding web viewers in standalone native 
(i.e., desktop-style) applications.  In this manner, authors/editors can preview the appearance 
...
\p{The value of indexes for enhancing local text search may certainly be extended to 
more general web/database searches.}

\p{Because \PacTk{} represents indexes via metadata, it is straightforward to merge 
indexes from multiple documents into an aggregate database, or meta-index.  Meta-indexes 
...
ways.  On the one hand, users might enter search terms and query against an overall 
...
querying for results in a hosted meta-index.  \lPacTk{} provides native \API{} tools 
so that \PDF{} viewers can query against a database when expanding local  
...
and other publishers' web content.}
...
\p{The \PacTk{} code base also includes prototype Cloud-Native service implementations 
for hosting meta-index databases.  These cloud applications --- which can be 
developed and tested as self-contained desktop components before being 
...
cloud-based meta-indexes need to compile machine-readable representations of 
text document contents.  This process also corrects for the 
...
combine two different target origins: on the other hand, text spans are declared 
...
of the document text as understood by human authors and readers.  At the same time, 
...
the document metadata can be consulted as a basis for searches both locally 
and via \API{}s.}
...
button-click.  Figure~\ref{fig:ss6} shows a range-select option that syncs mouse drags with key-presses to select a multi-line area that extends to the margins in the middle lines (a useful alternative to ordinary \PDF{} ranges that can only be a simple rectangular box, thereby either cutting important words from left or right or including extraneous words around the range's start and end).  Advanced \GUI{} programming in conjunction with machine-readable 
text encoding allows users to accomplish their goals in find, copy, and remote-search 
functionality with less hassle than traditional \PDF{} viewers (which rely on flawed internal text representation).}
...
research in to \SARSCoV{}.  The Allen Institute for \AI{} (\AItwo{}), for example, compiled a 
...
breakdown of natural-language text in paragraphs and sentences.  \lAItwo{} argued that new 
...
document-preparation system that generated structured text representations alongside 
conventional \PDF{} files.}

\p{\lPacTk{}-enabled documents can utilize similar encoding to expose full text content, 
alongside index metadata, to bibliographic databases.  Rigorous encoding allows 
...
representation of a \q{sentence,} as well as adjust for anomalies such as intraword 
...
should be included.  Rhetorical features such as footnotes, quotes, figure illustrations, 
enumerations, and itemized lists all represent discursive structures that complicated the 
simplistic breakdown of text into sequences of paragraphs and sentences.  Enhanced 
search, in particular, should allow queries to filter search targets into 
footnote text, section/chapter titles, quotes, content within figures/diagrams, 
and annotations/comments, as well as \q{main} document text.  Query protocols 
...
\vspace*{5pt}
%\sectnum{1.}
\hspace{2pt}\secttitle{{Data Publishing}}
\vspace*{5pt}
{\semihr{0.13\textwidth}}
...
For users intending to examine published data sets, raw data files are 
...
criteria).  Consequently, specific scientific disciplines are often 
...
\DICOM{} and \OMETIFF{} for bioimaging; and \GIS{}-indexed attribute layers and shapefiles for geospatial data sets, 
among many others.}
...
serialization protocols, so that custom parsers are needed to consume byte or character 
streams from raw data files.  Even when the actual encoding is performed via a 
generic standard like \XML{}, dedicated deserialization code is still necessary --- 
generic parsers can convert a given \XML{} document to a Document Object Model 
(\DOM{}) infoset, but algorithms need to navigate the document hierarchy and extract 
...
versioning conflicts, programming language barriers, operating-system 
...
\p{Embedding deserialization code is a good illustration of \FAIR{}sharing prioties: 
ideally, all the resources needed to (re)use a data set should be provided as 
...
or separate windows, to show the attributes of individual data objects.  For 
purposes of discussion, we refer to such self-contained resources specific to 
...
accessed interoperability from \PDF{} (or \SVG{}) files via the same conventions 
...
within image series with coded labels linked to published raw data; 
...
color- and/or shape-coded attribute layers for \GIS{} overlays which 
allow users to browse between digital maps and structured object 
...
interoperate.  With these cross-references, users should be able 
to navigate from a window dedicated to one media/file to a 
window rendering a different format, and correctly isolate the 
portion of a file (not just the overall file) embodied in a cross-reference.  
For example, geotagging video frames should allow users to pause a video 
...
remediation site).  Conversely, \GIS{} attribute layers can represent 
...
ensured proper interoperability between multimedia displays 
...
\p{Standardized cross-referencing protocols can allow scientific 
applications to interoperate with the range of assets that 
may be published alongside research works, as supplemental 
materials.  Rather than relying on generic \PDF{} viewers, scientists 
...
\lBdQS{} in particular, more that conventional languages, 
...
\vspace*{6pt}
%\sectnum{1.}
\hspace{2pt}\secttitle{{Documenting Data Profiles via Code Annotations and Conventions}}
\vspace*{4pt}
{\semihr{0.513\textwidth}}
\vspace*{7pt}
...
\p{In particular, \q{design by contract} programming techniques can be 
particularly valuable in scientific-computing contexts.  These techniques 
explicate what may otherwise be merely implicit technical and theoretical 
...
(e.g., that two lists have the same number of elements), alogorithm 
...
or code library serves to illustrate the theoretical 
...
can be studied interactively more so that quantitative 
...
\p{For more information, please see our software demo videos:
