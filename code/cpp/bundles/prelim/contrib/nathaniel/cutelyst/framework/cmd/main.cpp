...
bool _copy_qdir_recursive(QString source, QString dest)
{
 bool result = false;
 ...
 for(int i = 0; i < files.count(); i++)
 {
  ...
  result = QFile::copy(src_name, dest_name);
  if(!result)
    return false;
 }
...
bool copy_qdir_recursive(QDir parent, QString source, QString dest)
{
 return _copy_qdir_recursive(parent.absoluteFilePath(source), parent.absoluteFilePath(dest));
}
...
bool build_tsi(const QString &filename, const QString &appName)
{
 ...
 bool ok = false;
 if(qd.exists("tsi"))
   ok = copy_qdir_recursive(qd, "tsi", "tsi.backup");
 else
   ok = qd.mkpath("tsi"_qt);

 if(!ok)
 {
  qDebug() << "Trouble making and/or copying tsi directory; trying to continue ...";
 }
...
# argument unless this script is called with one or more alternative parameters)";
   ...
   // //  don't forget to leave a newline at the start of this string ...
   QString docker_lib_supplement = R"(
...
# THIS_APP_EXTRA_LIBS=$$PIN_ROOT_DIR/-build_/install/lib/%3-plugins/grantlee/%5:${PIN_ROOT_DIR}/-build_/grantlee/install/lib
#
...
#
#  With a build via cmake
# %6/lib/cutelyst%3-qt%4-plugins/grantlee/%1:%5/-build_/grantlee/install/lib
#
#  With a build via Qt Creator
# THIS_APP_EXTRA_LIBS=%2/lib/cutelyst%3-qt%4-plugins/grantlee/%1:%5/-build_/grantlee/via-qtc/lib
...
cp ./src/%1 ../../lib  \
...

 {
  ...
  if((ok = data.open(QFile::WriteOnly)))
  {
   QTextStream out(&data);
   out << R"(
The tsi folder will be regenerated if the generator is rerun.
If this folder already exists, the old folder will be renamed
   tsi.backup.  Note that any folder already named tsi.backup
   will be overwritten.
The tsi folder will be duplicated with the name tsi.orig.
   The idea is to preserve the autogenerated version of
   the folder and all its contents while the main tsi
   folder is modified.  Note that regenerating the tsi
   folder will also overwrite tsi.orig.
)";
   data.close();
  }
 }
 ...
 copy_qdir_recursive(qd, "tsi", "tsi.orig");
