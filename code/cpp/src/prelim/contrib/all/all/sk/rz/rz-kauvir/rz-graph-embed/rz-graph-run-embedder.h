...
public:
 ...
 RZ_Graph_Run_Embedder(caon_ptr<RZ_ASG_Valuer> valuer);
 void init_noop_node();
 ...
 void check_redirect(RZ_ASG_Result_Holder& rh, tNode& current_start_node);

 void redirect_to_noop(tNode& n);

 void redirect_to_noop(RZ_ASG_Result_Holder& rh, tNode& current_start_node);
 ...
 void redirect_with_call_no_package(RZ_ASG_Result_Holder& rh, tNode& redirect_entry_node,
  std::initializer_list<QString> args, caon_ptr<tNode> function_node = nullptr);

 void redirect_no_package(RZ_ASG_Result_Holder& rh, std::initializer_list<QString> args,
  caon_ptr<tNode> function_node = nullptr);

 void redirect(RZ_ASG_Result_Holder& rh, std::initializer_list<QString> args,
  caon_ptr<tNode> function_node = nullptr);

 void redirect(RZ_ASG_Result_Holder& rh, QStringList& qsl, tNode& function_node);

 void redirect(RZ_ASG_Result_Holder& rh, QString function_name, tNode& function_node);

 void embed_rename(RZ_ASG_Result_Holder& rh,
  QString function_name, tNode& function_node);

 void embed_deferred_callback(RZ_ASG_Result_Holder& rh,
  QString function_name, tNode& function_node);
