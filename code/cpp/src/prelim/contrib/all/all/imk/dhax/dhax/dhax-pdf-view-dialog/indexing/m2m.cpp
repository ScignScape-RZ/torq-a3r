...
QString Page_Ref::to_simple_text() const
{
 ...
 {
  if(secondary_note)
    return "%1 notes %2-%3"_qt.arg(number).arg(note).arg(secondary_note);
  if(note)
    return "%1 note %2"_qt.arg(number).arg(note);
...
void Index_Entry::add_redirect(QString text, QString ksupplement, QVector<Index_Entry>& entries)
{
 ...
 if(ix != -1)
 {
  ...
  entries.push_back({0, 0, 0, key, {}, carried, 0});
 }
 else
 {
  entries.push_back({0, 0, 0, text, {}, {}, 0});
 }
}
...
void read_index_entry(QString text, Index_Entry& ie)
{
 ...
 ie.sub_count = sub_count_token.toInt(); // + 1;  //somehow we're off by 1 ... not anymore
...
void ref_pairs_to_string_list(const QVector<Page_Ref_Pair>& prs,
  QVector<QPair<Page_Ref_Pair, QString>>& texts)
{
 for(const Page_Ref_Pair& pr : prs)
 {
  texts.push_back({pr, pr.to_code()});
 }
}
