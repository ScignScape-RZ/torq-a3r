...
bool PDF_Document_Widget::setDocument(const QString &filePath)
{
 ...
 if (doc)
 {
  ...
  for(int i = 0;i < doc->numPages(); i++)
  {
   ...
   for(Poppler::Annotation *ann : p->annotations())
   {
    if(ann->subType() == Poppler::Annotation::AHighlight)
    {
     if(!ann->contents().isEmpty())
       pdf_annotations_[i].push_back({ann->contents(), ann->boundary()});
    }
    //qDebug() << ann->boundary();
   }
...
void PDF_Document_Widget::setPage(int page)
{
 if (page != currentPage + 1)
 {
  ...
  Poppler::Page* popg = doc->page(page - 1);


  auto& vec = pdf_annotations_.value(page - 1);

  for(auto& pr : vec)
  {
   QRectF qrf = pr.second;
...
//   qreal scale_factor =  scale();
   ...
   quint64 tlx_pg = tlx * popg->pageSizeF().width() * scale_factor;
   quint64 tly_pg = tly * popg->pageSizeF().height() * scale_factor;

   quint64 brx_pg = brx * popg->pageSizeF().width() * scale_factor;
   quint64 bry_pg = bry * popg->pageSizeF().height() * scale_factor;
